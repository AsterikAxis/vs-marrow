...
class KeyBindMenu extends FlxSubState
{
    ...
    var keyEnd:Array<String> = ["LEFT ARROW", "DOWN ARROW", "UP ARROW", "RIGHT ARROW", "null       "];
    ...
    // I'll program this some other time.
    // var keysUI:Array<String> = [FlxG.save.data.leftUI, FlxG.save.data.downUI, FlxG.save.data.upUI, FlxG.save.data.rightUI];
    ...
    function textUpdate(){
        ...
        for(i in 0...5)
        {
            ...
            keyTextDisplay.text += textStart + keyText[i] + ": " + ((keys[i] != keyText[i]) ? (keys[i] + " / ") : "" ) + keyEnd[i] + "\n";
        }
    ...
    function save(){
        ...
        /*
        FlxG.save.data.upUI = keysUI[2];
        FlxG.save.data.downUI = keysUI[1];
        FlxG.save.data.leftUI = keysUI[0];
        FlxG.save.data.rightUI = keysUI[3];
        */
