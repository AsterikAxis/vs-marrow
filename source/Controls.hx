...
class Controls extends FlxActionSet
{
	...
	/*
	public function loadUIBinds()
	{
		removeKeyboard();
		KeyBinds.keyCheck();
	
		inline bindKeys(Control.UP, [FlxKey.fromString(FlxG.save.data.upUI)]);
		inline bindKeys(Control.DOWN, [FlxKey.fromString(FlxG.save.data.downUI)]);
		inline bindKeys(Control.LEFT, [FlxKey.fromString(FlxG.save.data.leftUI)]);
		inline bindKeys(Control.RIGHT, [FlxKey.fromString(FlxG.save.data.rightUI)]);
		inline bindKeys(Control.ACCEPT, [Z, SPACE, ENTER]);
		inline bindKeys(Control.BACK, [BACKSPACE, ESCAPE]);
	}
	*/
